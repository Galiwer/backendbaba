...
function getMySQLConnectionString() returns string {
    ...
    string? nodeEnv = os:getEnv("NODE_ENV");
    if nodeEnv is string && nodeEnv == "development" {
        io:println("WARNING: No MySQL environment variables found. Please set up your database connection.");
        io:println("For local development, you can:");
        io:println("1. Install MySQL locally");
        io:println("2. Set environment variables: MYSQL_HOST=localhost, MYSQL_USER=root, MYSQL_PASSWORD=your_password, MYSQL_DATABASE=babadb");
...
function getMySQLConnectionStringNoDB() returns string {
    ...
    string? nodeEnv = os:getEnv("NODE_ENV");
    if nodeEnv is string && nodeEnv == "development" {
        io:println("WARNING: No MySQL environment variables found for bootstrap connection.");
        io:println("For local development, you can:");
        io:println("1. Install MySQL locally");
        io:println("2. Set environment variables: MYSQL_HOST=localhost, MYSQL_USER=root, MYSQL_PASSWORD=your_password");
        io:println("3. Or use the provided env.local.dev file");
    }
...
public function main() {
    io:println("Starting Health Records Backend...");
    ...
    if dbClientResult is error {
        ...
        io:println("Trying bootstrap approach...");
        
        // Fallback to bootstrap approach
        ...
        mysql:Client|error dbClientResult2 = new (getMySQLConnectionStringNoDB());
        if dbClientResult2 is error {
            io:println("Database connection failed after bootstrap: ", dbClientResult2.message());
            return;
        }
        mysql:Client dbClient = dbClientResult2;
        ...
        io:println("Direct database connection successful!");
        mysql:Client dbClient = dbClientResult;
        globalClient = dbClient;
        
        // Test the connection
        sql:ParameterizedQuery testQuery = `SELECT 1 as test`;
        stream<record {}, sql:Error?> testStream = dbClient->query(testQuery);
        record {}|sql:Error? testRow = testStream.next();
        if testRow is record {} {
            io:println("Database connection test successful!");
        } else {
            io:println("Database connection test failed: ", testRow);
        }
    }
